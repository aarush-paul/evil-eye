...
check_update(){
	...
	relase_url='https://api.github.com/repos/aarush-paul/evil-eye/releases/latest'
	new_version=$(curl -s "${relase_url}" | grep '"tag_name":' | awk -F\" '{print $4}')
	tarball_url="https://github.com/aarush-paul/evil-eye/archive/refs/tags/${new_version}.tar.gz"
	...
	if [[ $new_version != $__version__ ]]; then
		...
		--retry 3 --retry-delay 2 --location --output ".evil-eye.tar.gz" "${tarball_url}"

		if [[ -e ".evil-eye.tar.gz" ]]; then
			tar -xf .evil-eye.tar.gz -C "$BASE_DIR" --strip-components 1 > /dev/null 2>&1
			[ $? -ne 0 ] && { echo -e "\n\n${RED}[${WHITE}!${RED}]${RED} Error occured while extracting."; reset_color; exit 1; }
			rm -f .evil-eye.tar.gz
			popd > /dev/null 2>&1
			{ sleep 3; clear; banner_small; }
			echo -ne "\n${GREEN}[${WHITE}+${GREEN}] Successfully updated! Run evil-eye again\n\n"${WHITE}
			{ reset_color ; exit 1; }
		else
...
about() {
	{ clear; banner; echo; }
	cat <<- EOF
		${GREEN} Author   ${RED}:  ${ORANGE}Aarush Paul${RED}[ ${ORANGE}@aarush-paul${RED}]
		${GREEN} Github   ${RED}:  ${CYAN}https://github.com/aarush-paul
		${GREEN} Social   ${RED}:  ${CYAN}https://aarush-paul.github.io
